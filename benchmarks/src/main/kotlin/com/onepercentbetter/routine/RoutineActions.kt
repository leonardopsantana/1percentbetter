...
package com.onepercentbetter.routine
...
fun MacrobenchmarkScope.routineWaitForContent() {
    ...
    val obj = device.waitAndFindObject(By.res("routine:topicSelection"), 10_000)
 ...
 */
fun MacrobenchmarkScope.routineSelectTopics(recheckTopicsIfChecked: Boolean = false) {
    val topics = device.findObject(By.res("routine:topicSelection"))
    ...
    while (visited < 3) {
        if (topics.childCount == 0) {
            fail("No topics found, can't generate profile for Routine page.")
        }
...
fun MacrobenchmarkScope.routineScrollFeedDownUp() {
    val feedList = device.findObject(By.res("routine:feed"))
